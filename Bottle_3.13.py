...
try:
    ...
    st.info(f"Ultralytics version: {ultralytics_version}")
except ImportError:
    st.info("Ultralytics version: unknown")
...
if uploaded_file and not stop_button:
    ...
    while cap.isOpened() and not stop_button:
        ...
        try:
            ...
            for r in results:
                # Handle different result formats in newer versions
                if hasattr(r, 'boxes') and r.boxes is not None:
                    ...
                    boxes = r.pred[0][:, :4].cpu().numpy()
                    classes = r.pred[0][:, 5].cpu().numpy()
                    confs = r.pred[0][:, 4].cpu().numpy()
